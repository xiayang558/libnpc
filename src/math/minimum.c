...
static long double min_long_double(long double a, long double b) {
    if (isnan(a) || isnan(b)) return NAN;
    return (a < b) ? a : b;
}
static complex double min_complex128(complex double a, complex double b) {
    double ma = cabs(a), mb = cabs(b);
    return (ma <= mb) ? a : b;
}
static complex float min_complex64(complex float a, complex float b) {
    float ma = cabsf(a), mb = cabsf(b);
    return (ma <= mb) ? a : b;
}
static complex long double min_complex256(complex long double a, complex long double b) {
    long double ma = cabsl(a), mb = cabsl(b);
    return (ma <= mb) ? a : b;
}

Array* minimum(Array *x1, Array *x2) {
    if (x1 == NULL || x2 == NULL) {
        fprintf(stderr, "minimum: NULL array argument\n"); return NULL;
    }
    if (x1->dtype != x2->dtype) {
        fprintf(stderr, "minimum: data type mismatch\n"); return NULL;
    }
    int out_ndim;
    int *out_shape = broadcast_shapes(2, (Array*[]){x1, x2}, &out_ndim);
    if (!out_shape) { fprintf(stderr, "minimum: broadcast failed\n"); return NULL; }
    Array *result;
    if (out_ndim == 0) {
        ...
        result->dtype = x1->dtype; result->shape = NULL; result->ndim = 0;
        result->size = 1; result->strides = NULL; result->is_view = 0;
    } else {
    ...
    char *data1 = (char*)x1->data, *data2 = (char*)x2->data, *res_data = (char*)result->data;
    int *indices = result->ndim ? calloc(result->ndim, sizeof(int)) : NULL;
    if (result->ndim && !indices) {
        free(strides1); free(strides2); free(res_strides); free_array(result); return NULL;
    }
    ...
    for (int flat = 0; flat < result->size; ++flat) {
        ...
        switch (x1->dtype) {
            case INT8: {
                ...
                int8_t res = (v1 < v2) ? v1 : v2; memcpy(pdst, &res, elem_sz); break;
            }
            case INT16: {
                int16_t v1 = *(int16_t*)p1, v2 = *(int16_t*)p2;
                int16_t res = (v1 < v2) ? v1 : v2; memcpy(pdst, &res, elem_sz); break;
            }
            case INT32: {
                int32_t v1 = *(int32_t*)p1, v2 = *(int32_t*)p2;
                int32_t res = (v1 < v2) ? v1 : v2; memcpy(pdst, &res, elem_sz); break;
            }
            case INT64: {
                int64_t v1 = *(int64_t*)p1, v2 = *(int64_t*)p2;
                int64_t res = (v1 < v2) ? v1 : v2; memcpy(pdst, &res, elem_sz); break;
            }
            case UINT8: {
                uint8_t v1 = *(uint8_t*)p1, v2 = *(uint8_t*)p2;
                uint8_t res = (v1 < v2) ? v1 : v2; memcpy(pdst, &res, elem_sz); break;
            }
            case UINT16: {
                uint16_t v1 = *(uint16_t*)p1, v2 = *(uint16_t*)p2;
                uint16_t res = (v1 < v2) ? v1 : v2; memcpy(pdst, &res, elem_sz); break;
            }
            case UINT32: {
                uint32_t v1 = *(uint32_t*)p1, v2 = *(uint32_t*)p2;
                uint32_t res = (v1 < v2) ? v1 : v2; memcpy(pdst, &res, elem_sz); break;
            }
            case UINT64: {
                uint64_t v1 = *(uint64_t*)p1, v2 = *(uint64_t*)p2;
                uint64_t res = (v1 < v2) ? v1 : v2; memcpy(pdst, &res, elem_sz); break;
            }
            case FLOAT32: {
                float v1 = *(float*)p1, v2 = *(float*)p2;
                float res = min_float(v1, v2); memcpy(pdst, &res, elem_sz); break;
            }
            case FLOAT64: {
                double v1 = *(double*)p1, v2 = *(double*)p2;
                double res = min_double(v1, v2); memcpy(pdst, &res, elem_sz); break;
            }
            case FLOAT128: {
                long double v1 = *(long double*)p1, v2 = *(long double*)p2;
                long double res = min_long_double(v1, v2); memcpy(pdst, &res, elem_sz); break;
            }
            case COMPLEX64: {
                complex float v1 = *(complex float*)p1, v2 = *(complex float*)p2;
                complex float res = min_complex64(v1, v2); memcpy(pdst, &res, elem_sz); break;
            }
            case COMPLEX128: {
                complex double v1 = *(complex double*)p1, v2 = *(complex double*)p2;
                complex double res = min_complex128(v1, v2); memcpy(pdst, &res, elem_sz); break;
            }
            case COMPLEX256: {
                complex long double v1 = *(complex long double*)p1, v2 = *(complex long double*)p2;
                complex long double res = min_complex256(v1, v2); memcpy(pdst, &res, elem_sz); break;
            }
            default:
                fprintf(stderr, "minimum: unsupported data type\n");
                free(strides1); free(strides2); free(res_strides);
                if (indices) free(indices); free_array(result); return NULL;
        }
    }
    free(strides1); free(strides2); free(res_strides);
