...
/** Linear interpolation: given sorted slice of length N and quantile q (0-1) */
static double qtl_value(double *sorted, int n, double q) {
    double idx_f = q * (n - 1);
    int lo = (int)idx_f;
...
Array* quantile(Array *a, Array *q, int axis) {
    if (a == NULL || q == NULL) {
        fprintf(stderr, "quantile: NULL argument\n");
        return NULL;
    }
    if (q->ndim != 1) {
        fprintf(stderr, "quantile: q must be 1D array\n");
        return NULL;
    }
    int ndim = a->ndim;
    if (axis < -1 || axis >= ndim) {
        fprintf(stderr, "quantile: invalid axis %d for %dD array\n", axis, ndim);
        return NULL;
    }
    ...
    if (axis == -1) {
        ...
        for (int i = 0; i < nq; i++) rd[i] = qtl_value(vals, n, qd[i]);
        free_array(dbl);
        return result;
    }
    ...
    for (int s = 0; s < num_slices; s++) {
        ...
        for (int qi = 0; qi < nq; qi++) {
            res_data[out_base + qi * res_strides[0]] =
                qtl_value(slice_vals, axis_len, qd[qi]);
        }
