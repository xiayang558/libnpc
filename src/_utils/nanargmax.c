...
Array* nanargmax(Array *a, int axis) {
    if (a == NULL) {
        fprintf(stderr, "nanargmax: NULL array argument\n");
        return NULL;
    }
    int ndim = a->ndim;
    if (axis < -1 || axis >= ndim) {
        fprintf(stderr, "nanargmax: invalid axis %d for %dD array\n", axis, ndim);
        return NULL;
    }
    ...
    if (axis == -1) {
        ...
        Array *res = create_scalar_array(&max_idx, INT64);
        return res;
    }
