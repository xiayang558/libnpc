...
/**
 * nanmax  --  compute array maximum, ignore NaN (like numpy.nanmax)
 *
 * @param a    Input array
...
Array* nanmax(Array *a, int axis) {
    if (a == NULL) {
        fprintf(stderr, "nanmax: NULL array argument\n");
        return NULL;
    }
    int ndim = a->ndim;
    if (axis < -1 || axis >= ndim) {
        fprintf(stderr, "nanmax: invalid axis %d for %dD array\n", axis, ndim);
        return NULL;
    }
    ...
    if (axis == -1) {
        ...
        for (int i = 0; i < a->size; ++i) {
            ...
            if (!found || val > max_val) { max_val = val; found = 1; }
        }
        /* all NaN → return NaN */
        Array *res;
        if (!found) {
            ...
            res = create_scalar_array(&max_val, FLOAT64);
        }
        return res;
    }
    ...
    for (int slice = 0; slice < num_slices; ++slice) {
        ...
        res_data[out_off] = found ? max_val : NAN;
