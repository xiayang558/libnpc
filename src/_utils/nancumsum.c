...
Array* nancumsum(Array *a, int axis) {
    if (a == NULL) {
        fprintf(stderr, "nancumsum: NULL array argument\n");
        return NULL;
    }
    int ndim = a->ndim;
    if (axis < -1 || axis >= ndim) {
        fprintf(stderr, "nancumsum: invalid axis %d for %dD array\n", axis, ndim);
        return NULL;
    }
        ...
        if (is_complex) {
            ...
            for (int i = 0; i < a->size; ++i) {
                ...
                if (!is_nan_complex(ptr, a->dtype)) {
                    sum += get_complex_val(ptr, a->dtype);
                }
                void *out_ptr = dst + i * out_elem;
                if (out_dtype == COMPLEX64) {
                    *(complex float*)out_ptr = (complex float)sum;
                } else {
                    *(complex double*)out_ptr = sum;
                }
            }
        } else {
            double sum = 0.0;
            for (int i = 0; i < a->size; ++i) {
                ...
                if (!is_nan_real(ptr, a->dtype)) {
                    sum += get_real_val(ptr, a->dtype);
                }
                *(double*)(dst + i * out_elem) = sum;
            }
        }
        return result;
    }
    ...
    for (int slice = 0; slice < num_slices; ++slice) {
        ...
        if (is_complex) {
            ...
            for (int k = 0; k < axis_len; ++k) {
                ...
                *(double*)(dst + (base + k * strides[axis]) * out_elem) = sum;
            }
        }
