...
Array* all(Array *arr, int axis, int keepdims) {
    if (!check_null(arr, "all")) return NULL;
    if (!check_axis(arr, axis, "all")) return NULL;
    ...
    if (axis == -1) {
        ...
        if (shape) free(shape);
        if (result == NULL) {
            utils_error_log("Error", "all", "Failed to create result array");
            return NULL;
        }
        *(uint8_t*)result->data = all_true ? 1 : 0;
        return result;
    }
    ...
    if (out_shape == NULL) {
        utils_error_log("Error", "all", "Memory allocation failed");
        return NULL;
    }
    int out_idx = 0;
    ...
    while (1) {
        ...
        res_data[res_flat] = all_true ? 1 : 0;
